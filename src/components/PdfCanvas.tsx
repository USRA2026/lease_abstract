...
export default function PdfCanvas({ fileUrl, pageNumber, rects, renderWidth = 700 }: PdfCanvasProps) {
  ...
  return (
    <Document
      ...
      error={<div className="p-8 text-sm text-rose-500">Failed to load this document.</div>}
    >
      <div className="relative inline-block bg-white shadow-lg">
        ...
        {rects.map((r, i) => (
          <div
            key={i}
            className="pointer-events-none absolute rounded-sm bg-amber-300/50 ring-2 ring-amber-400"
            style={{
              left: r.x * scale,
